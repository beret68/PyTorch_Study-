...
class FashionMNISTDataModule:
    def __init__(self, data_dir: str, batch_size: int, device: torch.device):
    ...
    def get_validation_loader(self) -> DeviceDataLoader:
        loader = torch.utils.data.DataLoader(self.validation, batch_size=self.batch_size, shuffle=False)
        return DeviceDataLoader(loader, self.device)