...
class ResNextBLK(nn.Module):
    def __init__(
        ...
        self.conv2 = nn.Conv2d(
            ...
            stride=strides,
            padding=1,
            groups=bottleneck_channels//width_of_group
        )
...
class AnyNet(nn.Module):
    def __init__(
        ...
        self.net = nn.Sequential(
            self.stem(input_channels, stem_channels), self.body(arch)
        )
        self.net.add_module(
            "head",
            nn.Sequential(
    ...
    def stage(
        ...
        for i in range(depth):
            if i ==0:
                ...
                ) #except for the first block in a stage
...
class RegNet(AnyNet):
    def __init__(
        ...
        for depth, in_channels, out_channels in zip(depths, in_channels, out_channels):
            arch.append(
                (depth, in_channels, out_channels, groups_width, bottleneck_multiplier)
            )
        super().__init__(arch, input_channels, stem_channels, num_classes)